...
<div class="create-post-card" onclick="document.getElementById('postModal').style.display='flex'">
    ...
    <div class="open-modal-btn">{{ Auth::user()->name }} ơi, bạn đang nghĩ gì thế?</div>
        ...
        <div class="post-feed">
            ...
            {{-- 1. THÊM DATA-ID VÀO ĐÂY --}}
            <div class="post-card" data-id="{{ $post->id }}">
<div class="ins-header">
    {{-- Đổi asset('images/default-avatar.png') thành biến động --}}
    ...
    <i class="fas fa-ellipsis-h ms-auto"></i>
                ...
                <div class="post-image-container">
                    @if($post->image_url)
                        {{-- 2. THÊM ONDBLCLICK VÀO ẢNH --}}
                        <img src="{{ asset($post->image_url) }}" class="post-image" ondblclick="handleLike({{ $post->id }})">
                    @else
                        <div class="no-image-placeholder">Không có ảnh</div>
                    @endif
                    {{-- 3. THÊM ID CHO TIM TRẮNG --}}
